<script lang="ts">
  ...
  const confidentLines: SongLine[] = [
    ...
    },
    {
      ...
      target: 'whakapono',
      aliases: ['faka pono', 'waka pono', 'wakapono', 'fakopono'],
      start: 10.2,
      maoriWord: 'whakapono',
      syllables: ['wha', 'ka', 'po', 'no'],
    },
    {
      maori: 'Me te rangimārie',
      english: 'Peace',
      target: 'rangimaarie',
      aliases: ['rangi marie', 'rangi mary', 'rangimarie', 'rangimaari'],
      start: 15,
      maoriWord: 'rangimārie',
      syllables: ['ran', 'gi', 'mā', 'rie'],
    },
    {
      maori: 'Tātou, tātou e',
      english: 'All of us',
      target: 'tatou',
      aliases: ['tātou', 'ta tou', 'taatou', 'tattoo', 'tatu', 'ta toe', 'all of us'],
      start: 20,
      maoriWord: 'tātou',
      syllables: ['tā', 'tou'],
    },
  ...
  async function startTrySinging() {
    ...
    try {
      ...
      const text = await transcribeWithGroq(blob, {
        targetWord: currentLine.maoriWord,
      })

      micState = 'idle'
      micLevel = 0

      const lvl = scoreAttempt(
        text,
        {
          lyric: currentLine.maori,
          maoriWord: currentLine.maoriWord,
          aliases: currentLine.aliases,
          english: currentLine.english,
          color: currentLine.colour ?? '#cccccc',
  ...
  function browserFallbackSinging() {
    ...
    recognition.onresult = (event: any) => {
      ...
      const words = transcript.split(/\s+/).map(norm)
      ...
      for (const w of words) {
        for (const t of targets) {
          ...
          const maxD = t.length <= 4 ? 2 : 3
          if (levDist(w, t) <= maxD) close = true
        }
      ...
      if (!exact) {
        for (const t of targets) {
          if (full.includes(t) || levDist(full, t) <= 3) {
            exact = true
            break
          }
        }
      ...
      const lvl: AttemptLevel = exact || close ? (exact ? 'exact' : 'fuzzy') : voiceHeard ? 'attempt' : 'silent'
  ...
  if (!giveCredit) {
    if (r.level === 'silent') {
      feedback = `AI check: Try saying "${currentLine.maoriWord}" out loud.`
    } else {
      feedback = `AI check: Nearly there! Try saying "${currentLine.maoriWord}" again.`

      if (soundIsOn()) {
        // Beginner uses short line clips.
        // Confident uses Te Aroha audio replay from the correct timestamp.
        if (level === 'confident') {
          replayLine()
        } else {
          playClipOrSpeak(lineClips[currentIdx] as ClipKey, currentLine.maori)
        }
      }
    }
  ...
  feedback = passed
    ? r.level === 'exact'
      ? `AI check: Ka pai! You said "${currentLine.maoriWord}" clearly.`
      : `AI check: Close enough! Ka pai!`
    : `AI check: Ka pai for trying! Let's move on!`
 ...
 function handleMicError(err: unknown) {
    ...
    if (msg.includes('NotAllowed') || msg.includes('denied')) {
      micError = "I couldn't open the microphone. Please allow mic access and try again."
      return
    }
  ...
  async function getAIHintFromGroq() {
    ...
    const prompt = `
You are helping a Year 1-3 child learn a Māori waiata.

Give ONE very short, safe, child-friendly hint.

Rules:
- Do not invent new Māori words.
- Do not change the lyric.
- Do not give a long explanation.
- Use simple English.
- Help the child pronounce or understand the current word.
- Keep it under 18 words.

Context:
Level: ${level}
Current lyric: ${currentLine.maori}
Meaning: ${currentLine.english}
Key Māori word: ${currentLine.maoriWord}
Syllables: ${currentLine.syllables.join(' - ')}
Has tried singing: ${hasTried ? 'yes' : 'no'}
Help count: ${helpCount}
Last heard: ${lastAttempt?.heard ?? 'nothing yet'}
`
    ...
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You create short, safe, child-friendly learning hints for Māori waiata practice.',
          },
          {
            role: 'user',
            content: prompt,
    ...
    if (!res.ok) {
      throw new Error('Groq hint request failed')
    }
  ...
  async function getHint() {
    ...
    hintLoading = true
    showAIHintBox = true
    aiHintText = 'AI hint is thinking...'

    const w = currentLine.maoriWord
    const breakdown = currentLine.syllables.join(' - ')

    let hint: string

    try {
      hint = await getAIHintFromGroq()
    } catch {
      if (helpCount === 1 && !lastAttempt) {
        hint = `Listen to Kiki again, then say "${w}" slowly.`
      } else if (helpCount <= 2) {
        hint = `Try small parts: ${breakdown}. Then sing the line.`
      } else if (helpCount === 3) {
        hint = `First say "${w}". Then say: ${currentLine.maori}.`
      } else {
        hint = `Sing with Kiki. Listen, then say "${w}" the same way.`
      }
    }
    ...
    if (soundIsOn()) {
      const u = new SpeechSynthesisUtterance(aiHintText)
      u.lang = 'en-NZ'
...
<div class="page">
  <div class="stage" style="background-image: url({bgImg})">
    <button class="top-map" onclick={() => { stopAllSound(); cancelMic(); push('/') }}>
      ← Back to Map
    </button>
    ...
    <div class="action-layout-container">
      <div class="action-row">
        <button
          class="action sing state-{buttonState}"
          class:recording={micState === 'recording'}
          onclick={startTrySinging}
          disabled={micState === 'transcribing'}
          aria-label={micState === 'recording' ? 'Listening — tap to stop' : micState === 'transcribing' ? 'Recognising…' : 'Try singing'}
        >
          {#if micState === 'recording'}
            <span class="meter">
              {#each Array(5) as _, i}
        ...
        <button
          ...
          onclick={getHint}
          disabled={micState !== 'idle' || hintLoading}
          aria-label="Need help — get an AI hint"
        >
      ...
      {:else if lastAttempt}
        <div class="feedback-card" class:good={lastAttempt.matched} class:retry={!lastAttempt.matched}>
          <img
            src={lastAttempt.matched ? kiwiYes : kiwiTryAgain}
            alt={lastAttempt.matched ? 'Ka pai!' : 'Try again'}
            class="fb-kiwi"
          />

          <div class="fb-text">
            <p class="fb-main">{feedback}</p>
          </div>
        </div>
      {:else}
...
.need-help-img {
  ...
  .action.sing.state-listening {
    background: linear-gradient(180deg, #fb7185, #e11d48);
    animation: pulse 1s ease-in-out infinite;
  }

  .action.sing.state-recognizing {
    background: linear-gradient(180deg, #b9a89a, #826a55);
    cursor: progress;
  }

  .action.sing.state-passed {
    background: linear-gradient(180deg, #5bd35f, #2e9d33);
  }

  .kiwi.bouncing {
    animation: kibounce .5s ease;
  }
