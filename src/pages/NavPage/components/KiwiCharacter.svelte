...
<style>
  .kiwi {
    position: absolute; width: 6.2%;
    transform: translate(-50%, -86%);
    z-index: 20; pointer-events: none; will-change: left, top;
  }
  .kiwi img {
  ...
  .kiwi .shadow {
    position: absolute; left: 50%; bottom: -6%; width: 78%; aspect-ratio: 3/1;
    transform: translateX(-50%);
    background: radial-gradient(50% 50% at 50% 50%, rgba(0,0,0,.4), transparent 72%);
    z-index: -1;
  }
  .kiwi .shadow.walking { animation: shadowPulse .36s ease-in-out infinite; }
  .bob { transform-origin: 50% 100%; animation: breathe 3.2s ease-in-out infinite; }
  .bob.walking { animation: bob .36s ease-in-out infinite; }
  ...
  @keyframes shadowPulse {
    ...
    50%     { transform: translateX(-50%) scale(.78); opacity: .6; }
  }

  @media (prefers-reduced-motion: reduce) {
    .bob, .bob.walking, .shadow.walking { animation: none !important; }
  }
</style>