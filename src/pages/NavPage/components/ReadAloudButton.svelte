...
<style>
  .readme {
    position: absolute; left: 2.4%; bottom: 4.5%; z-index: 40;
    display: inline-flex; align-items: center; gap: .5em;
    padding: .55em 1em .55em .7em;
    border: 2px solid rgba(255,255,255,.55); border-radius: 999px;
    cursor: pointer; color: #15364a;
    font-family: inherit; font-weight: 700; font-size: clamp(11px,1.7vmin,17px);
  ...
  .readme svg  { width: 1.3em; height: 1.3em; flex: none; }
  .readme .wave { opacity: 0; transition: opacity .2s ease; }
  .readme.on   { background: linear-gradient(180deg, #ffe6a8, #f4c25c); }
  .readme.on .wave { opacity: 1; animation: pulse 1s ease-in-out infinite; }
  .readme:hover:not(:disabled)  { transform: translateY(-2px); box-shadow: 0 10px 18px rgba(0,0,0,.4), 0 1px 0 rgba(255,255,255,.6) inset; }
  .readme:active:not(:disabled) { transform: translateY(0); }
  .readme:disabled               { opacity: .5; cursor: not-allowed; }
  .readme:focus-visible          { outline: 3px solid #ffe9a8; outline-offset: 3px; }

  @keyframes pulse { 0%,100% { opacity: .5; } 50% { opacity: 1; } }

  @media (prefers-reduced-motion: reduce) {
    .readme.on .wave { animation: none !important; }
  }
</style>