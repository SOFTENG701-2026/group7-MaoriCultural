...
<style>
  .guide-root { position: absolute; inset: 0; z-index: 45; }
  ...
  .banner {
    position: absolute; left: 50%; bottom: 4%; width: 58%;
    ...
    animation: rise .32s cubic-bezier(.34,1.56,.64,1) both;
  }

  .line {
    position: absolute; left: 20%; right: 4.5%; top: 0%; bottom: 30%;
    display: flex; align-items: center; justify-content: center;
    text-align: center; font-weight: 700; line-height: 1.25;
    font-size: clamp(13px,2.5vmin,24px);
    animation: swap .25s ease-out;
  }

  .footer {
    position: absolute; left: 20%; right: 5%; bottom: 20%;
    display: flex; align-items: center; justify-content: space-between;
  }

  .dots { display: inline-flex; gap: .4em; }
  .dot  {
    width: clamp(5px,.9vmin,8px); height: clamp(5px,.9vmin,8px);
    border-radius: 50%; background: rgba(90,53,20,.28);
    transition: background .2s ease, transform .2s ease;
  }
  .dot.on { background: #b3722a; transform: scale(1.25); }

  .cue {
    display: inline-flex; align-items: center; gap: .2em;
    font-weight: 800; font-size: clamp(14px,2.2vmin,20px); color: #8a4a16;
  }
  .cue svg { width: 1.05em; height: 1.05em; animation: nudge 1.1s ease-in-out infinite; }

  .skip {
    pointer-events: auto;
    padding: .1em .2em; border: 0; background: transparent; cursor: pointer;
    color: #8a4a16; font-family: inherit; font-weight: 800;
    font-size: clamp(14px,2.2vmin,20px); letter-spacing: .3px;
    transition: color .16s ease, transform .16s ease;
  }
  .skip:hover       { color: #5a3514; transform: scale(1.06); }
  .skip:focus-visible { outline: 3px solid #d97c1d; outline-offset: 2px; border-radius: 8px; }

  .catcher:focus-visible { outline: 3px solid #ffe9a8; outline-offset: -6px; border-radius: 16px; }

  @keyframes rise  { from { opacity:0; transform:translateX(-50%) translateY(14px); } to { opacity:1; transform:translateX(-50%) translateY(0); } }
  @keyframes swap  { from { opacity:0; transform:translateY(4px); } to { opacity:1; transform:translateY(0); } }
  @keyframes nudge { 0%,100% { transform:translateX(0); } 50% { transform:translateX(3px); } }

  @media (prefers-reduced-motion: reduce) {
    .banner, .line, .cue svg { animation: none !important; }
  }
</style>