<!--
  Pepeha Module — Page 5: Find the Awa

  Layout copied from Page 4 (PepehaMaungaPage). The learner finds the river among
  three cards (mountain / river / people), reveals the selected school's awa, and
  adds the Awa sticker to Kiki's board.

  Cultural safety: the awa shown comes from the chosen *practice-school* example
  (pepehaState.selectedSchool) — never invented or guessed.

  Confident level adds a short cultural-safety check ("What should Kiki remember
  about awa in pepeha?") as a modal triggered when the learner taps Whānau Circle
  to move on — it must be answered correctly before the page advances.

  Layout (over the scene background):
    • Title + speaker (top-centre)            • Settings (top-right)
    • Kiki + speech bubble (left)
    • Notebook with question + 3 cards (centre)
    • Reveal panel: "You found the awa!" + sticker card + Add Sticker (right)
    • Bottom bar: Listen · Need help? · Check answer · Whānau Circle
-->
<script lang="ts">
  ...
  import titleImg       from '../../assets/pepeha/page5/Title.png'
  import whanauCircleImg from '../../assets/pepeha/page5/Circle.png'
  ...
  // Per-awa reveal stickers — fixed art per school (keyed by school id).
  import whauSticker      from '../../assets/pepeha/page5/whau_awa_sticker_transparent.png'
  import momutuSticker    from '../../assets/pepeha/page5/momutu_awa_sticker_transparent.png'
  import waikumeteSticker from '../../assets/pepeha/page5/waikumete_awa_sticker_transparent.png'
  import kahikaSticker    from '../../assets/pepeha/page5/kahika_awa_sticker_transparent.png'
  ...
  // The awa to reveal comes from the chosen practice-school example.
  ...
  // The reveal sticker is fixed art per school (keyed by school id).
  const AWA_STICKERS: Record<string, string> = {
    'rosebank':   whauSticker,
    'don-buck':   momutuSticker,
    'glen-eden':  waikumeteSticker,
    'kauri-park': kahikaSticker,
  }
  const stickerImg = $derived(AWA_STICKERS[pepehaState.selectedSchoolId ?? ''] ?? whauSticker)

  const title = 'Find the Awa'
  ...
  // Three tappable cards. 'river' is the awa.
  ...
  const stickerAdded = $derived(pepehaState.hasSticker('awa'))
  ...
  function checkAnswer() {
    ...
    checked = true
    if (selectedCard === 'river') {
      found = true
      speak(`You found the awa! ${awa}. ${awaLine}`)
    } else {
      speak('Try again. Awa means river.')
    }
  }

  function addSticker() {
    pepehaState.addSticker('awa')
    speak('Awa sticker added!')
  }

  const wrongHint = $derived(checked && selectedCard !== 'river')

  // Confident-level short check — triggered when the learner taps Whānau Circle
  // to move on. They must answer correctly before the page advances.
  // Cultural-safety focus: awa is part of place connection, not any water.
  const QUIZ = {
    question: 'What should Kiki remember about awa in pepeha?',
    options: [
      'Awa connects to place',
      'Awa means any water',
      'Awa should be guessed',
    ],
    correctIndex: 0,
  }
  ...
  // Whānau Circle: Beginner (or once the check is passed) advances straight
  // away; Confident must first answer the short cultural-safety check.
  function handleNext() {
  ...
  function checkQuiz() {
    ...
    if (pickedOption === QUIZ.correctIndex) {
      quizPassed = true
      speak('Ka pai! Awa is part of place connection.')
      setTimeout(() => {
        showQuiz = false
        onNext()
      }, 900)
  ...
  const readText = $derived(
    showQuiz
      ...
      : `${title}. ${kikiLine} Which one is the awa?`
  )
</script>
...
<div class="stage" style="background-image:url({bgImg})">
  ...
  <div class="notebook">
    <p class="question">
      Which one is the <strong>awa</strong>?
    </p>
    <ul class="cards" aria-label="Find the awa">
      {#each CARDS as card}
        <li>
          <button
            class="card"
            class:selected={selectedCard === card.id}
            class:correct={found && card.id === 'river'}
            class:wrong={wrongHint && selectedCard === card.id}
            onclick={() => pickCard(card.id)}
            aria-pressed={selectedCard === card.id}
            aria-label={`${card.alt}${found && card.id === 'river' ? ', correct answer' : wrongHint && selectedCard === card.id ? ', incorrect answer, try again' : ''}`}
          >
            {#if found && card.id === 'river'}
    ...
    {#if wrongHint}
      <p class="hint" role="status" aria-live="polite">Try again. Awa means river.</p>
    {/if}

    <!-- Action buttons sit directly below the option cards -->
    <div class="actions">
      <button class="img-btn" onclick={() => speak(`${awa}. ${awaLine}`)} aria-label="Listen">
        <img src={listenImg} alt="Listen" />
      </button>
      <button class="img-btn" onclick={() => speak('Awa means river. Follow the water.')} aria-label="Need help?">
  ...
  <!-- Reveal panel — right (shown once the river is found) -->
  {#if found}
    <div class="reveal" role="status">
      <div class="sticker-card" style="background-image:url({stickerBgImg})">
        <img src={foundBannerImg} alt="You found the awa!" class="reveal-banner" />
        <img src={stickerImg} alt={`Awa Sticker. ${awa}. ${awaLine}`} class="reveal-card" />
      </div>
      {#if stickerAdded}
        <img src={addedPillImg} alt="Awa sticker added!" class="reveal-added" />
      {:else}
      ...
      <!-- Whānau Circle appears once the awa is found, below the reveal panel -->
      <button class="img-btn next-btn" onclick={handleNext} aria-label="Whānau Circle">
        <img src={whanauCircleImg} alt="Whānau Circle" />
      </button>
    </div>
  {/if}

  <!-- Confident-level short check — appears once the awa is found -->
  {#if showQuiz}
    <div class="quiz-overlay" role="dialog" aria-modal="true" aria-label={QUIZ.question}>
      <div class="quiz-card">
        <p class="quiz-line">{awaLine}</p>
...
<style>
  ...
  /* Per-awa reveal sticker (fixed art selected from AWA_STICKERS) */
  .reveal-card {
    width: 100%;
    height: auto;
    display: block;
  ...
  /* Whānau Circle — image button, appears once the awa is found */
  .next-btn img { width: min(210px, 23vw); }
  /* Gentle breathing pulse to draw the eye once Whānau Circle appears */
  .next-btn { animation: breathe 1.8s ease-in-out infinite; }
  @keyframes breathe {
